...
export class PlayMode implements Mode<ModeType.Play> {
    ...
    readonly previewPosition = new LiveValue<null | Vector2>(
        null,
        "PlayMode.previewPosition",
    );
    ...
    toDebugStringLive(): string {
        return debugStateToString("play", {
            _: this.previewPosition.live()?.toString(2) ?? null,
        });
    }
    onPointerEvent({
        event,
        eventType,
        splatapus,
    }: PointerEventContext<PointerEventType>): void {
        switch (eventType) {
            case "move":
            case "down":
                if (event.isPrimary) {
                    this.previewPosition.update(
                        splatapus.viewport.eventSceneCoords(event),
                    );
                }
                return;
            case "cancel":
