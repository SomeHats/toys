...
export function RightBar({ splatapus }: { splatapus: Splatapus }) {
    ...
    const keyFrameIndex = Array.from(keyPoints).findIndex(
        (keyPoint) => keyPointId === keyPoint.id,
    );
    const shapeIndex = Array.from(shapes).findIndex(
    ...
    return (
        <div className="relative flex h-0 flex-auto flex-col gap-4 overflow-auto p-5 [-webkit-overflow-scrolling:touch]">
            ...
            {shapes.map((shape, i) => (
                <div
                    className="relative flex w-max flex-col gap-2"
                    key={shape.id}
                >
                        ...
                        {keyPoints.map((keyPoint) => (
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    splatapus.location.keyPointId.update(
                                        keyPoint.id,
                                    );
                                }}
                            >
                        ...
                        <button
                            ...
                            onClick={() => {
                                ...
                                splatapus.document.update(
                                    (document) =>
                                        document.addKeyPoint(keyPointId, null),
                                    {
            ...
            <Button
                ...
                onClick={() => {
                    ...
                    splatapus.document.update(
                        (document) => document.addShape(shapeId),
                        {
                            lockstepLocation: (location) => ({
                                ...location,
                                shapeId,
