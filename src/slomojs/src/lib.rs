...
#[wasm_bindgen]
pub fn start(source: String, element: web_sys::HtmlElement) -> Result<(), JsValue> {
   ...
   let mut slomo = t::SloMoJs::new(document.clone())?;
   let _script: t::Script = slomo.add_node(&script)?;

   log!("slomo: {:#?}", slomo);
   log!("out: {}", slomo);

   element.append_child(slomo.get_container())?;
